...
module.exports = class deviceLED {
  ...
  async internalBrightnessUpdate (value, callback) {
    if (this.accessory.getService(this.Service.Lightbulb).getCharacteristic(this.Characteristic.Brightness).value === value) {
      callback()
      return
    }
    callback()
  ...
  async internalColourUpdate (value, callback) {
    if (this.accessory.getService(this.Service.Lightbulb).getCharacteristic(this.Characteristic.Hue).value === value) {
      callback()
      return
    }
    callback()
    try {
      ...
      if (this.platform.debug) {
        this.platform.log('[%s] updating hue to [%s°] and sat to [%s%].', this.accessory.displayName, value, curSat)
      }
    } catch (err) {
      this.platform.log.warn('[%s] could not be updated and its status will be reverted soon. Error: %s.', this.accessory.displayName, err.message)
    }
