...
export default class {
  constructor(platform, accessory) {
    ...
    this.isLANOnly = !accessory.context.useAWSControl && !accessory.context.useBLEControl;
    ...
    [
      ...
      if (confCode?.sceneCode) {
        const { sceneCode } = confCode;

        // Add to the global enabled scenes list
        this.usedCodes[sceneCode] = charName;
        ...
        if (confCode?.showAs === 'switch') {
          ...
          this.accessory.getService(charName)
            ...
            .onSet(async (value) => {
              await this.internalSceneUpdate(charName, sceneCode, value, true);
            })
            .updateValue(false);
        } else {
          ...
          this.service
            ...
            .onSet(async (value) => {
              await this.internalSceneUpdate(charName, sceneCode, value);
            })
            .updateValue(false);
        }
      } else {
    ...
    try {
      ...
      if (this.hasScenes) {
        setTimeout(() => {
          ...
          Object.values(this.usedCodes).forEach((cn) => {
            if (this.service.testCharacteristic(this.cusChar[cn])) {
              this.service.updateCharacteristic(this.cusChar[cn], false);
            }
            if (this.accessory.getService(cn)) {
              this.accessory.getService(cn).updateCharacteristic(this.hapChar.On, false);
            }
          });
        }, 1000);
      }
  ...
  async internalSceneUpdate(charName, code, value, isService = false) {
    try {
