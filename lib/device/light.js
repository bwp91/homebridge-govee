...
export default class {
  constructor(platform, accessory) {
    ...
    [
      ...
      if (confCode?.sceneCode) {
        ...
        const codeParts = confCode.sceneCode.split('||');
  ...
  externalUpdate(params) {
    ...
    // Only log scene codes if the device is actually on
    if (this.cacheState === 'on') {
      ...
      (params.commands || []).forEach((command) => {
        ...
        switch (deviceFunction) {
          ...
          case '23ff': // Maybe used for the Timer functions
            ...
            break;
          default:
            this.accessory.log(`${platformLang.newScene}: [${hexString}] [${params.baseCmd}:aa] (may or may not work)`);
            break;
        }
      });
    }
