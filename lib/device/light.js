...
export default class {
  constructor(platform, accessory) {
    ...
    [
      ...
      if (confCode) {
        ...
        // Check the first two characters of the code are 33, and it has an overall length of 40
        if (confCode.substring(0, 2) === '33' && confCode.length === 40) {
        ...
        accessory.logWarn(`[${confCode}] is not a valid code - please update with a new code from the log that starts with 33`);
      }
  ...
  externalUpdate(params) {
    ...
    (params.commands || []).forEach((command) => {
      ...
      if (firstPart === '33') {
        this.accessory.logDebugWarn(`${platformLang.newScene}: [${command}] [${hexString}] [${params.baseCmd}:33]`);
      } else if (firstPart === 'a3') {
        this.accessory.logDebugWarn(`${platformLang.newScene}: [${command}] [${hexString}] [${params.baseCmd}:a3]`);
          ...
          if (this.cacheScene !== actionCode) {
            if (this.cacheState === 'on') {
              ...
              if (hasProperty(this.usedCodes, this.cacheScene)) {
                this.accessory.log(`${platformLang.curScene} [${this.usedCodes[this.cacheScene]} - ${this.cacheScene}] [${params.baseCmd}:aa]`);
              } else {
                this.accessory.log(`${platformLang.curScene} [${this.cacheScene}] [${params.baseCmd}:aa]`);
              }
            } else {
              this.cacheScene = '';
            }
