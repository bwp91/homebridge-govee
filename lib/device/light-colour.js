...
module.exports = class deviceLightColour {
  constructor (platform, accessory) {
    ...
    this.isRGBIC = deviceConf && deviceConf.isRGBIC
    ...
    this.accessory.context.effectSwitches = {}
    this.log('[%s] %s %s.', this.name, 'Adding switches', deviceConf.lightEffects)
    for (const [key, scene] of Object.entries(this.cusScenes.rgbicAWSScenes)) {
      const serviceName = this.name + " " + scene['label']
      this.log('[%s] %s %s.', this.name, 'lightEffects', key)
      if (this.lightEffects[key] || key === 'colorMode') {
        this.log('[%s] %s %s.', this.name, 'addLinkedAccessory', serviceName)
        let switchAccessory = this.accessory.addLinkedAccessory(accessory.context.device, serviceName)
        switchAccessory.addService(this.hapServ.Switch)
        this.log('[%s] %s %s.', this.name, 'Create Service', key)
        this.accessory.context.effectSwitches[key] = switchAccessory.getService(this.hapServ.Switch)
            .updateCharacteristic(this.hapChar.Name, serviceName)
            .getCharacteristic(this.hapChar.On)
            .onSet(async value => {
              await this.internalEffectUpdate(serviceName, key, scene['command'], value)
            })
            .updateValue(key !== 'colorMode')
      }
    }
    ...
    this.sceneChars = []
    if (deviceConf) {
      ;['diyMode', 'diyModeTwo', 'musicMode', 'musicModeTwo', 'scene', 'sceneTwo'].forEach(
        scene => {
          ...
          if (deviceConf[scene]) {
            ...
            if (
              (firstChar === '0' && accessory.context.useBLEControl) ||
              (['M', 'o'].includes(firstChar) && accessory.context.useAWSControl)
            ) {
          ...
          if (this.service.testCharacteristic(this.cusChar[charName])) {
            this.service.removeCharacteristic(
              this.service.getCharacteristic(this.cusChar[charName])
            )
          }
        }
      )
    ...
    const opts = JSON.stringify({
      ...
      enableAWS: accessory.context.hasAWSControl
  ...
  async internalEffectUpdate(serviceName, key, command, value) {
    try {
      if (!value && key === 'colorMode') {
        return;
      }
      ...
      for (const [effectKey, effectSwitch] in this.accessory.context.effectSwitches) {
        if (key === effectKey) {
          continue
        }

        effectSwitch.updateCharacteristic(this.hapChar.On, false)
      }
