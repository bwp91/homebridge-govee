...
module.exports = class deviceLightColour {
  constructor (platform, accessory) {
    ...
    const deviceOb = platform.lightDevices[deviceId]
    this.alShift = deviceOb && deviceOb.adaptiveLightingShift
      ? deviceOb.adaptiveLightingShift
      : platform.consts.defaultValues.adaptiveLightingShift
    this.brightStep = deviceOb && deviceOb.brightnessStep
      ? Math.min(deviceOb.brightnessStep, 100)
      : platform.consts.defaultValues.brightnessStep
    this.disableDeviceLogging = deviceOb &&
      this.funcs.hasProperty(deviceOb, 'overrideDisabledLogging')
      ? !deviceOb.overrideDisabledLogging
    ...
    if (
      ...
      if (this.debug) {
        this.log('[%s] %s [+%s].', this.name, this.messages.alSetupShift, this.alShift)
      }
    }
  }
