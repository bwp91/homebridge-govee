...
module.exports = class deviceLightColour {
  ...
  async internalOnOffUpdate (value, callback) {
    try {
      ...
      // Log a warning if the device is offline
      if (this.cacheOnline === 'false') {
        this.log.warn('[%s] %s.', this.name, this.messages.offlineTry)
      }
  ...
  externalUpdate (newParams) {
    ...
    if (newParams.online !== this.cacheOnline) {
      this.cacheOnline = newParams.online

      // Log the change, at a warning level if the device is reported offline
      if (this.cacheOnline === 'true') {
        this.log('[%s] %s.', this.name, this.messages.online)
      } else {
        this.log.warn('[%s] %s.', this.name, this.messages.offline)
      }
    }
