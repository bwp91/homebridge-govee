...
module.exports = class deviceLightColour {
    ...
    */
    if (this.exposeScenes) {
      ...
      this.sceneList.forEach(scene => {
        ...
        sceneService.getCharacteristic(this.hapChar.On).onSet(async (value, scene) => {
          await this.internalSceneUpdate(value, scene)
        })
  ...
  async internalOnOffUpdate (value) {
    try {
      ...
      if (!this.cacheOnline || !this.accessory.context.controllable) {
  ...
  internalSceneUpdate (value, scene) {
    try {
