...
module.exports = class deviceLightColour {
  constructor (platform, accessory) {
    ...
    this.accessory.removeService(this.hapServ.Switch)
    for (const [key, scene] of Object.entries(this.cusScenes.rgbicAWSScenes)) {
      ...
      if (this.lightEffects[key] || key === 'colorMode') {
        this.log('[%s] %s %s.', this.name, 'effectService', serviceName)
        let effectSwitch = new this.hapServ.Switch(serviceName, this.platform.api.hap.uuid.generate(key), key)
        ...
        this.accessory.context.effectSwitches[key] = effectSwitch
        this.accessory.addService(effectSwitch)
      }
    }
