...
module.exports = class deviceLightColour {
  constructor (platform, accessory) {
    ...
    const opts = JSON.stringify({
      ...
      bluetooth: accessory.context.isBLEDevice
        ? accessory.context.btControl
          ? 'enabled'
          : 'disabled'
        : 'unsupported',
  ...
  externalUpdate (newParams) {
    ...
    if (this.enableDebugLogging && this.firstUpdateDone) {
      this.log(
        '[%s] %s [%s] [%s].',
        this.name,
        this.updateTimeout ? this.lang.ignoringUpdate : this.lang.receivingUpdate,
        newParams.source,
    ...
    if (newParams.source === 'api' && this.updateTimeout) {
      return
    }
    ...
    if (newParams.brightness && newParams.brightness !== this.cacheBrightRaw) {
      // Brightness is different so update Homebridge with new values
      this.cacheBrightRaw = newParams.brightness
    ...
    if (newParams.colorTem || newParams.color) {
      ...
      if (newParams.colorTem) {
        kelvin = Math.max(newParams.colorTem, 140)
