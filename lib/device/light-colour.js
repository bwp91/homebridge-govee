...
export default class {
  constructor(platform, accessory) {
    ...
    if (
      ...
      this.accessory.log(`${platformLang.noticeAWS} [${accessory.context.gvModel}]`);
    }
  ...
  async internalStateUpdate(value) {
    try {
      ...
      this.updateTimeoutLAN = timerKey;
      setTimeout(() => {
      ...
      setTimeout(() => {
        if (this.updateTimeoutLAN === timerKey) {
          this.updateTimeoutLAN = false;
        }
      }, 5000);
  ...
  async internalSceneUpdate(charName, code, value) {
    try {
      ...
      this.accessory.log(`${platformLang.curScene} [${charName}]`);
  ...
  externalUpdate(params) {
    ...
    if (
      ...
      || (this.updateTimeoutLAN && params.source === 'LAN')
    ) {
      return;
    }
    ...
    if (params.kelvin || params.rgb) {
      ...
      if (colourChange) {
        ...
        if (this.alController?.isAdaptiveLightingActive?.() && sigColourChange) {
          this.alController.disableAdaptiveLighting();
    ...
    if (params.scene) {
      ...
      if (!this.usedCodes.includes(fullCode)) {
        this.accessory.logDebugWarn(`${platformLang.sceneCode}:\n${fullCode}`);
      }
    }
  }
}
