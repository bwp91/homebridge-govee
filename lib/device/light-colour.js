...
module.exports = class deviceLightColour {
  constructor (platform, accessory) {
    ...
    this.lang = platform.lang.zDeviceControl
    this.funcs = platform.funcs
    this.colourUtils = platform.colourUtils
    this.hapError = platform.api.hap.HapStatusError
    this.hapServ = platform.api.hap.Service
    this.hapChar = platform.api.hap.Characteristic
    // this.cusChar = platform.cusChar
    this.noScale = platform.consts.noScale
    this.updateAccessoryStatus = platform.updateAccessoryStatus
    ...
    try {
      ...
      if (!this.cacheOnline) {
        if (this.forceUpdates) {
          ...
          throw new Error(this.lang.devNotCont)
        }
      }
  ...
  async internalBrightnessUpdate (value) {
    try {
      ...
      const newBrightness = this.noScale.includes(this.accessory.context.gvModel)
        ? value
  ...
  externalUpdate (newParams) {
    ...
    if (this.debug && this.firstUpdateDone) {
      ...
      this.log('[%s] %s [%s].', this.name, text, JSON.stringify(newParams))
    }
    ...
    if (this.funcs.hasProperty(newParams, 'online')) {
      ...
      if (this.cacheOnline !== status) {
        ...
        this.updateAccessoryStatus(this.accessory, this.cacheOnline)
      }
    }
