...
module.exports = class deviceLightColour {
    ...
    if (Object.keys(this.lightEffects).length !== 0) {
      ...
      for(const [effectKey, effectEnabled] of Object.entries(this.lightEffects)) {
        ...
        switchService
            ...
            .updateValue(false)
        this.accessory.context.effectServices[effectCommand] = switchService
  ...
  async internalEffectUpdate(key, command, value) {
    try {
      ...
      for (const [effectKey, effectSwitch] of Object.entries(this.accessory.context.effectServices)) {
        if (key === effectKey) {
          continue
        }
  ...
  externalUpdate (params) {
    ...
    if (params.command) {
      let allOff = true
      for (const [effectCommand, effectService] of Object.entries(this.accessory.context.effectServices)) {
