...
module.exports = class deviceLightColour {
  ...
  async internalColourUpdate (value, callback) {
    try {
      ...
      const newRGB = this.colourUtils.hsl2rgb(value, currentSat)
      if (
      ...
      const testRGB = this.colourUtils.hsv2rgb(value, currentSat)
      ...
      // Time the logging with sending the new testing colour
      await this.funcs.sleep(7000)

      // Cache the new state and log if appropriate
      this.cacheR = testRGB[0]
      this.cacheG = testRGB[1]
      this.cacheB = testRGB[2]

      if (!this.disableDeviceLogging) {
        this.log(
          '[%s] %s using alternative [rgb %s %s %s].',
          this.name,
          this.messages.curColour,
          Math.round(testRGB[0]),
          Math.round(testRGB[1]),
          Math.round(testRGB[2])
        )
      }

      // End testing alternative conversion
  ...
  async internalCTempUpdate (value, callback) {
    try {
      ...
      const newRGB = this.colourUtils.hsl2rgb(hs[0], hs[1])
  ...
  externalUpdate (newParams) {
    ...
    if (newParams.colorTemInKelvin || newParams.color) {
      ...
      if (newParams.colorTemInKelvin) {
        ...
        rgb = this.colourUtils.hsl2rgb(hs[0], hs[1])
      } else {
