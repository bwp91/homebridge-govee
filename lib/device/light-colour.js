    ...
    try {
      ...
      if (!this.cacheOnline) {
        if (this.forceUpdates) {
          if (this.debug) {
            this.log('[%s] %s.', this.name, this.messages.sendingAnyway)
          }
        } else {
  ...
  async internalCTempUpdate (value) {
    try {
      ...
      if (this.accessory.context.supportedCmds.includes('colorTem')) {
        // Send a kelvin value between 2000 and 9000
        objToSend.name = 'colorTem'
        objToSend.value = Math.min(Math.max(k, 2000), 9000)
      } else {
        // Send rgb values
