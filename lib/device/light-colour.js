...
export default class {
  ...
  externalUpdate(params) {
    ...
    (params.commands || []).forEach((command) => {
      ...
      if (getTwoItemPosition(hexParts, 1) === '33') {
        const opt1 = `${command}||ptReal`;
        const opt2 = `${command}||pt||op`;
        const opt3 = `${command}||pt||opcode`;
        if (!this.usedCodes.includes(opt1) && !this.usedCodes.includes(opt2) && !this.usedCodes.includes(opt3)) {
          this.accessory.logWarn(`${platformLang.sceneCode}:\n${opt1}\n${opt2}\n${opt3}\nPlease try one of the above`);
        }
        return;
      }
