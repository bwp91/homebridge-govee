...
module.exports = class deviceLightColour {
  constructor (platform, accessory) {
    ...
    if (this.service.testCharacteristic(this.cusChar.Bluetooth)) {
      this.service.removeCharacteristic(this.service.getCharacteristic(this.cusChar.Bluetooth))
    }
    if (this.service.testCharacteristic(this.cusChar.BluetoothConn)) {
      this.service.removeCharacteristic(this.service.getCharacteristic(this.cusChar.BluetoothConn))
    }
  ...
  async internalBrightnessUpdate (value) {
    try {
      ...
      const newBrightness = this.consts.scaleBrightness.includes(this.accessory.context.gvModel)
  ...
  externalUpdate (params) {
    ...
    if (params.source === 'API' && this.updateTimeout) {
      return
    }

    // Check to see if the provided online status is different from the cache value
    if (this.funcs.hasProperty(params, 'online')) {
      const status = this.funcs.parseStatus(params.online)
      if (this.cacheOnline !== status) {
        this.cacheOnline = status
        this.platform.updateAccessoryAPIStatus(this.accessory, this.cacheOnline)
      }
    }

    // Check to see if the provided state is different from the cached value
    if (params.powerState && params.powerState !== this.cacheState) {
      // State is different so update Homebridge with new values
      this.cacheState = params.powerState
      this.service.updateCharacteristic(this.hapChar.On, this.cacheState === 'on')

      // Log the change if appropriate
      if (this.firstUpdateDone && this.enableLogging) {
        this.log('[%s] %s [%s].', this.name, this.lang.curState, params.powerState)
      }
    }

    // Check to see if the provided brightness is different from the cached value
    if (params.brightness && params.brightness !== this.cacheBrightRaw) {
      // Brightness is different so update Homebridge with new values
      this.cacheBrightRaw = params.brightness
      this.cacheBright = this.consts.scaleBrightness.includes(this.accessory.context.gvModel)
        ? Math.max(Math.min(Math.round(this.cacheBrightRaw / 2.54), 100), 0)
        : Math.max(Math.min(this.cacheBrightRaw, 100), 0)
      this.service.updateCharacteristic(this.hapChar.Brightness, this.cacheBright)
    ...
    if (params.colorTem || params.color) {
      ...
      let mired
      if (params.colorTem) {
        kelvin = Math.max(params.colorTem, 140)
