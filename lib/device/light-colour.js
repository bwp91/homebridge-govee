  ...
  constructor (platform, accessory) {
    ...
    for (const [key, scene] of Object.entries(this.cusScenes.rgbicAWSScenes)) {
      ...
      this.accessory.removeService(
          this.platform.cusServ.SceneService(
              this.name,
              key
          )
      )
      if (this.lightEffects[key] || key === 'colorMode') {
        ...
        let sceneService = this.platform.cusServ.SceneService(this.name, key)
            ...
            .updateValue(false)
        this.accessory.context.effectSwitches[key] = sceneService
        this.accessory.addService(sceneService)
      }
    }
