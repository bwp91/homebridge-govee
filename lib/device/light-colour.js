  ...
  constructor (platform, accessory) {
    ...
    */
    if (this.exposeScenes) {
      ...
      // Log that this is dev
      this.log.warn('[%s] ***************************************************************', this.name)
      this.log('[%s] scenes are in development and the buttons currently do nothing.', this.name)
      this.log.warn('[%s] ***************************************************************', this.name)
      ...
      this.sceneList.filter(scene => !this.accessory.getService(scene))
        .forEach(scene => {
          // Create a version of the name in lowercase with no spaces for the subtype
          const lowerForm = scene.toLowerCase().replace(/[\s]+/g, '')
          this.accessory.addService(this.hapServ.Lightbulb, scene, lowerForm)
          this.log('[%s] added service for scene [%s].', this.name, scene)
        })

      // Now the scene list and services should match up, log to confirm
      this.accessory.services.filter(service => service.displayName)
        .forEach(service => {
          this.log.warn(
            '[%s] has scene service [%s (%s)].',
            this.name,
            service.displayName,
            service.subtype
          )
