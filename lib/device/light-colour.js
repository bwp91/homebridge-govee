...
export default class {
  constructor(platform, accessory) {
    ...
    const opts = JSON.stringify({
      ...
      bluetooth: accessory.context.hasBLEControl ? useBLEControl : 'unsupported',
  ...
  async internalStateUpdate(value) {
    try {
      ...
      const timerKey = platformFuncs.generateRandomString(5);
  ...
  async internalBrightnessUpdate(value) {
    try {
      ...
      const updateKeyBright = platformFuncs.generateRandomString(5);
      this.updateKeyBright = updateKeyBright;
      await platformFuncs.sleep(350);
  ...
  async internalColourUpdate(value, force = false) {
    try {
      ...
      const updateKeyColour = platformFuncs.generateRandomString(5);
  ...
  async internalCTUpdate(value) {
    try {
      // This acts like a debounce function when endlessly sliding the colour wheel
      const updateKeyCT = platformFuncs.generateRandomString(5);
  ...
  externalUpdate(params) {
    ...
    if (platformFuncs.hasProperty(params, 'brightness') && params.brightness !== this.cacheBrightRaw) {
