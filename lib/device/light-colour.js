...
module.exports = class deviceLightColour {
  constructor (platform, accessory) {
    ...
    for (const [key, scene] in Object.entries(this.cusScenes.rgbicAWSScenes)) {
      const serviceName = this.name + " " + scene['label']
      this.log('[%s] %s %s.', this.name, 'lightEffects', key)
      if (this.lightEffects[key] || key === 'colorMode') {
        this.log('[%s] %s %s.', this.name, 'Creat Service', key)
        let service = this.hapServ.Switch(serviceName, key)
        ...
        this.accessory.context.effectSwitches[key] = service
        this.log('[%s] %s %s.', this.name, 'addLinkedAccessory', serviceName)
        this.platform.addLinkedAccessory(accessory.context.device, serviceName)
            .addService(service)
      }
    }
