...
module.exports = class deviceLightColour {
  constructor (platform, accessory) {
    ...
    if (Object.keys(this.lightEffects).length !== 0) {
      ...
      for(const [effectKey, effectData] of Object.entries(this.lightEffects)) {
        let effectDevice = {
          id: masterDevice.device + "." + effectKey,
          model: masterDevice.model + " " + effectData.label + " effect",
          deviceName: masterDevice.deviceName + " " + effectData.label + " effect"
        }
        let effectCommand = JSON.stringify(effectData.command)
        let effectAccessory = this.platform.addLinkedAccessory(this.accessory, effectDevice)
        let switchService = (effectAccessory.getService(this.hapServ.Switch) ||
            effectAccessory.addService(this.hapServ.Switch))
        switchService
            .addOptionalCharacteristic(this.cusChar.EffectCommand)
        switchService
            .updateCharacteristic(this.cusChar.EffectCommand, effectCommand)
        switchService
            ...
            .updateValue(false)
        this.accessory.context.effectSwitches[effectCommand] = switchService
      }
    }
  ...
  externalUpdate (params) {
    ...
    if (params.command) {
      for (const [effectCommand, effectService] of Object.entries(this.accessory.context.effectSwitches)) {
        effectService.updateCharacteristic(this.hapChar.On, (params.command === effectCommand))
      }
    }
