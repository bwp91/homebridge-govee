...
export default class {
  constructor(platform, accessory) {
    ...
    if (this.service1) {
      this.service1.getCharacteristic(this.hapChar.On)
        .updateValue(false)
        .onSet(async (value) => this.internalStateUpdate(this.service1, value, '3305000200000000000000000000000000000034'));
    }
    ...
    if (this.service2) {
      this.service2.getCharacteristic(this.hapChar.On)
        .updateValue(false)
        .onSet(async (value) => this.internalStateUpdate(this.service2, value, '3305000300000000000000000000000000000035'));
    }
    ...
    if (this.service3) {
      this.service3.getCharacteristic(this.hapChar.On)
        .updateValue(false)
        .onSet(async (value) => this.internalStateUpdate(this.service3, value, '3305000400000000000000000000000000000032'));
    }
    ...
    if (this.service4) {
      this.service4.getCharacteristic(this.hapChar.On)
        .updateValue(false)
        .onSet(async (value) => this.internalStateUpdate(this.service4, value, '3305000500000000000000000000000000000033'));
    }
  ...
  async internalStateUpdate(service, value, hexCode) {
    try {
      if (!value) {
        return;
      }

      // The kettle is a weird one as we seem to need to change to the mode and then turn the device on

      // Send the request to the platform sender function to change the mode, we need to change the hexCode to base64
      await this.platform.sendDeviceUpdate(this.accessory, {
        cmd: 'ptReal',
        value: hexToBase64(hexCode),
      });

      // Not sure if this is necessary but wait a second
      await sleep(500);

      // Send the request to the platform sender function to turn on
      await this.platform.sendDeviceUpdate(this.accessory, {
        cmd: 'ptReal',
