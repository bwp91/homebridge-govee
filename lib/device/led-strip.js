...
module.exports = class deviceLEDStrip {
  ...
  async internalCTempUpdate (value, callback) {
    try {
      callback()
      // const conv = this.hapConvert.colorTemperatureToHueAndSaturation(value)
      // const hs = [conv.hue, conv.saturation]
      const conv = this.helpers.m2hs(value)
      if (!this.alController || (this.alController && !this.alController.isAdaptiveLightingActive())) {
        if (this.debug) this.log.warn('[%s] ct set: %s with AL disabled.', this.accessory.displayName, value)
        this.lightService.updateCharacteristic(this.Characteristic.Saturation, conv[1])
        this.lightService.setCharacteristic(this.Characteristic.Hue, conv[0])
        return
      }
      if (this.debug) this.log.warn('[%s] ct set: %s with AL enabled.', this.accessory.displayName, value)
      const newRGB = this.helpers.hs2rgb(conv)
      if (newRGB[0] === this.cacheR && newRGB[1] === this.cacheG && newRGB[2] === this.cacheB) return
      ...
      this.cacheG = newRGB[1]
      this.cacheB = newRGB[2]
      this.log.warn('[%s] current colour [rgb %s, %s, %s].', this.accessory.displayName, newRGB[0], newRGB[1], newRGB[2])
      this.skipNextUpdates = 2
    } catch (err) {
      this.log.warn('[%s] could not be updated and its status will revert soon - %s.', this.accessory.displayName, err.message)
    }
