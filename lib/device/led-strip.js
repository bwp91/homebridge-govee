...
module.exports = class deviceLEDStrip {
  ...
  async internalCTempUpdate (value, callback) {
    try {
      ...
      if (this.debug && this.alController && this.alController.isAdaptiveLightingActive()) {
        this.log('[%s] adaptive lighting [%s mired].', this.accessory.displayName, value)
      }
    ...
    if (newParams.colorTemInKelvin || newParams.color) {
      ...
      if (rgb[0] !== this.cacheR || rgb[1] !== this.cacheG || rgb[2] !== this.cacheB) {
        ...
        if (this.firstUpdateDone) {
          ...
          if (this.alController) {
            this.alController.disableAdaptiveLighting()
            if (this.debug) this.log('[%s] adaptive lighting disabled due to significant colour change.', this.accessory.displayName)
          }
        }
      }
    }
