...
export default class {
  ...
  async internalSpeedUpdate(value) {
    try {
      ...
      // Throw a 'no response' error and set a timeout to revert this after 2 seconds
      setTimeout(() => {
        this.service.updateCharacteristic(this.hapChar.On, parseInt(this.cacheSpeed.split('_')[1] ?? '0', 10) / 0.08);
      }, 2000);
      throw new this.hapErr(-70402);
    }
  }
    ...
    switch (params.scene) {
      case fanState.ON: {
        ...
        if (this.cacheState !== 'ON') {
          this.cacheState = 'ON';
          this.service.updateCharacteristic(this.hapChar.On, 1);
      ...
      case fanState.OFF: {
        ...
        if (this.cacheState !== 'OFF') {
          this.cacheState = 'OFF';
          this.service.updateCharacteristic(this.hapChar.On, 0);
