...
export default class {
  ...
  async internalSpeedUpdate(value) {
    try {
      ...
      const newValue = parseInt(value / 11, 10);
      const newMode = newValue === 99 ? 'auto' : 'manual';
      ...
      if (newMode === 'auto' && !this.accessory.context.sensorAttached) {
        throw new Error('auto mode not supported without a linked sensor');
      }
