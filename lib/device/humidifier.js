...
module.exports = class deviceHumidifier {
  constructor (platform, accessory) {
    ...
    this.cacheState = this.service
        .getCharacteristic(this.hapChar.Active)
        .onSet(async value => await this.internalStateUpdate(value))
        .value;

    this.cachePercent = this.service
        .getCharacteristic(this.hapChar.RelativeHumidityDehumidifierThreshold)
  ...
  async internalHumidityUpdate (value) {
    try {
      ...
      if (this.cacheState === 0) {
        await this.internalStateUpdate(1)
      }
      ...
      this.service
          .updateCharacteristic(this.hapChar.RelativeHumidityDehumidifierThreshold, newPercent)
  ...
  externalUpdate (params) {
    ...
    if (params.state && params.state !== this.cacheState) {
      ...
      if (this.service.getCharacteristic(this.hapChar.CurrentHumidifierDehumidifierState).value !== (this.cacheState ? 2 : 0)) {
        this.service.updateCharacteristic(this.hapChar.CurrentHumidifierDehumidifierState, this.cacheState ? 2 : 0)
      }
      if (this.service.getCharacteristic(this.hapChar.Active).value !== this.cacheState) {
        this.service.updateCharacteristic(this.hapChar.Active, this.cacheState)
      }
      ...
      if (Object.values(this.value2Code).includes(params.scene)) {
        Object.keys(this.value2Code).forEach((key, idx) => {
          if (this.value2Code[key] === params.scene) {
            ...
            if (this.service
                .getCharacteristic(this.hapChar.RelativeHumidityDehumidifierThreshold).value !== this.cachePercent) {
              this.service.updateCharacteristic(this.hapChar.RelativeHumidityDehumidifierThreshold, this.cachePercent)
            }
          }
        })
      } else {
