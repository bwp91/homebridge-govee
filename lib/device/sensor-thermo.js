...
export default class {
  ...
  async processTempRequestOrUpdate() {
    this.log('[%s] Cur state [%s]; asked to [%s]', this.name, this.cacheCurHeatingCoolingState, this.cachecacheTargetHeatingCoolingState);
  ...
  async updateCurStatus() {
    ...
    try {
      ...
      this.log('[%s] Queried to [%s] and the response was: %s', this.name, response.data.url, response.data);
      ...
      this.log.warn('[%s] Error getting status, respose was: %s', this.name, error.response.data);
  ...
  async turnAcOnOff(isOn) {
    ...
    try {
      ...
      this.log('[%s] response data url: %s', this.name, response.data.url);
      this.log('[%s] response data: %s', this.name, response.data);
    } catch (error) {
      this.log.warn('[%s] Error on turningonoff, respose was: %s', this.name, error.response.data);
    }
  }
}
