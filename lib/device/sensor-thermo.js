...
export default class {
  constructor(platform, accessory) {
    ...
    this.thermostatService.getCharacteristic(this.hapChar.CurrentHeatingCoolingState).onGet(async () => {await this.updateCurStatus(); return this.cacheCurHeatingCoolingState });
  ...
  async updateCurStatus() {
    ...
    try {
      ...
      this.log.warn('[%s] Error getting status, respose was: %s', this.name, error);
  ...
  async turnAcOnOff(isOn) {
    ...
    try {
      ...
      this.log.error('[%s] Error on turningonoff, respose was: %s', this.name, error);
    }
  }
}
