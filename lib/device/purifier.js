...
module.exports = class devicePurifier {
  constructor (platform, accessory) {
    ...
    this.speed2Value = speed => Math.min(Math.max(parseInt(speed / 25), 1), 4)
  ...
  async internalSpeedUpdate (value) {
    try {
      ...
      const updateKeyAWS = this.funcs.generateRandomString(5)
      this.updateKeyAWS = updateKeyAWS
      ...
      setTimeout(() => {
        if (this.updateKeyAWS === updateKeyAWS) {
