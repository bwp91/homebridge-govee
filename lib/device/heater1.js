    ...
    (params.commands || []).forEach((command) => {
      ...
      switch (deviceFunction) {
        ...
        case '1a01': { // Target temperature (thermostat mode on)
          const newMode = getTwoItemPosition(hexParts, 2) === '01' ? 'auto' : 'heat';
          if (this.cacheMode !== newMode) {
            this.cacheMode = newMode;
            this.service.updateCharacteristic(this.hapChar.TargetHeatingCoolingState, this.cacheMode === 'auto' ? 0 : 1);
