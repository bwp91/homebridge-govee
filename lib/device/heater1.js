...
export default class {
  constructor(platform, accessory) {
    ...
    if (this.accessory.context.noReportTemp) {
      // Add the fan service if it doesn't already exist
      this.service = this.accessory.getService(this.hapServ.Fan);

      if (!this.service) {
        this.service = this.accessory.addService(this.hapServ.Fan);
        this.service.addOptionalCharacteristic(this.hapChar.SwingMode);
        this.service.addOptionalCharacteristic(this.hapChar.LockPhysicalControls);
      }
    } else {
  ...
  async prepareAsFanOnly() {
    try {
      this.accessory.logWarn('preparing as fan only');

      // Remove the heater service
      this.accessory.removeService(this.service);

      // Remove the existing fan service
      this.accessory.removeService(this.fanService);

      // Warn to please restart homebridge
      this.accessory.logWarn('homebridge restart needed for this accessory to work properly');
    } catch (err) {
      // Catch any errors during the process
      this.accessory.logWarn(`${platformLang.devNotUpdated} ${parseError(err)}`);
    }
  ...
  externalUpdate(params) {
    ...
    if (hasProperty(params, 'temperature') && !this.accessory.context.noReportTemp) {
      ...
      if (newTemp !== this.cacheTemp) {
        if (newTemp > 100) {
          ...
          if (!this.accessory.context.noReportTemp) {
            this.accessory.context.noReportTemp = true;
            this.prepareAsFanOnly();
          }
        } else {
    ...
    if (hasProperty(params, 'setTemperature') && !this.accessory.context.noReportTemp) {
    ...
    (params.commands || []).forEach((command) => {
      ...
      switch (deviceFunction) {
        ...
        case '1a01': { // Target temperature (thermostat mode on)
          ...
          if (this.cacheMode !== newMode && !this.accessory.context.noReportTemp) {
            this.cacheMode = newMode;
