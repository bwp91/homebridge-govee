...
module.exports = class connectionHTTP {
  constructor (platform) {
    ...
    this.apiKey = platform.config.apiKey.toString().toLowerCase().replace(/[^a-z0-9-]+/g, '')
    this.debug = platform.config.debug
    this.log = platform.log
    this.parsedIgnoredDevices = []

    // *** Backwards Compatibility *** \\
    if (typeof platform.config.ignoredDevices === 'string' && platform.config.ignoredDevices.length > 0) {
      platform.config.ignoredDevices = platform.config.ignoredDevices.split(',')
    }
    // *** End *** \\

    if (Array.isArray(platform.config.ignoredDevices) && platform.config.ignoredDevices.length > 0) {
      platform.config.ignoredDevices.forEach(sn => {
  ...
  async getDevices () {
    ...
    if (body.data.devices && body.data.devices.length > 0) {
      body.data.devices.forEach(device => {
        if (!this.parsedIgnoredDevices.includes(device.device)) {
