...
const pfxToBundle = async (endpoint, accountId, p12, p12Password) => new Promise((resolve, reject) => {
  readPkcs12(p12, { p12Password }, (err, cert) => {
    if (err) {
      reject(err);
    }
    try {
      ...
      resolve({
        endpoint: endpoint,
        accountId: accountId,
        certificate: Buffer.from(cert.cert, 'utf8').toString('utf8'),
        privateKey: Buffer.from(key.exportKey('pkcs8'), 'utf8').toString('utf8'),
      });
    } catch (error) {
      reject(error);
    }
...
module.exports = class connectionHTTP {
  constructor (platform) {
    ...
    this.clientId = 'hb_' + platform.api.hap.uuid.generate(this.username).slice(0, 10)
  }

  async getIotData () {
    if (this.iotData !== undefined) {
      return this.iotData;
    }
    ...
    try {
      ...
      if (!res.data?.data || !res.data.data.p12 || !res.data.data.p12Pass || !res.data.data.endpoint) {
        throw new Error('Unable to retrieve AWS authentication from server.');
      }

      const p12 = Buffer.from(res.data.data.p12);
      const p12Password = res.data.data.p12Pass;

      return await pfxToBundle(res.data.data.endpoint, this.accountId, p12, p12Password);

    } catch(err) {
      throw new Error(`${this.lang.disableAWS} unable to retrieve authentication information from server.`)
    }
  }
  ...
  async login () {
    try {
      ...
      this.accountId = res.data.client.accountId;
