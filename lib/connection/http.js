...
module.exports = class connectionHTTP {
  ...
  async getDevices () {
    ...
    if (body.code !== 200 || !this.helpers.hasProperty(body, 'data')) {
      throw new Error(this.helpers.hasProperty(body, 'message') ? body.message : 'An unknown error occured [getDevices()].')
    }
    const deviceList = []
  ...
  async getDevice (accessoryContext) {
    ...
    if (body.code !== 200 || !this.helpers.hasProperty(body, 'data')) {
      throw new Error(this.helpers.hasProperty(body, 'message') ? body.message : 'An unknown error occured [getDevice()].')
    }
    if (!this.helpers.hasProperty(body.data, 'properties')) {
  ...
  async updateDevice (accessoryContext, cmd) {
    ...
    if (body.code !== 200 || !this.helpers.hasProperty(body, 'data')) {
      throw new Error(this.helpers.hasProperty(body, 'message') ? body.message : 'An unknown error occured [updateDevice()].')
    }
  }
}
