...
export default class {
  constructor(platform) {
    ...
    const clientSuffix = platform.api.hap.uuid
      .generate(this.username) // 36 characters
      .replace(/-/g, '') // 32 characters
      .substring(0, this.clientId.length - 2); // 30 characters

    this.clientId = `hb${clientSuffix}`; // 32 characters
  }
