...
export default class {
  constructor(platform, iotFile) {
    ...
    // Create buffer from certificate and key
    const keyBuffer = Buffer.from(iotFile.key, 'utf8');
    const certBuffer = Buffer.from(iotFile.certificate, 'utf8');

    this.device = iotDevice({
      privateKey: keyBuffer,
      clientCert: certBuffer,
  ...
  async requestUpdate(accessory) {
    ...
    const payload = {
      msg: {
        cmd: 'status',
        cmdVersion: accessory.context.awsStatusCode[0],
        transaction: `${accessory.context.awsStatusCode[1]}_${Date.now()}000`,
        type: 0,
      },
    };
