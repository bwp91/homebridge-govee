      ...
      if (body.code !== 200 || !body.data) {
        ...
        if (body.code === 429) {
          if (this.rateLimitMessageShown) {
            this.log.warn('Still receiving 429 [rate limite exceeded], trying again in 10 minutes...')
          } else {
            this.log.warn('The plugin has exhausted the new Govee API quota limitations.')
            this.log.warn('The plugin will continue to retry every 10 minutes until connection can be made.')
            this.log.warn('Unfortunately the plugin will not work until fully initialised.')
            this.log.warn('This should take less than 24 hours, until then your logs will be quite spammed by this plugin.')
