...
export default class {
  ...
  async getDevice(accContext) {
    try {
      ...
      const toReturn = {};
      body.data.properties.forEach((property) => Object.assign(toReturn, property));
      return toReturn;
    } catch (err) {
      if (err.response?.status === 429) {
  ...
  async updateDevice(accessory, params) {
    try {
      ...
      accessory.logDebug(`[API] ${platformLang.sendingUpdate} ${JSON.stringify({ name: params.cmd, value: params.data })}`);
