...
export default class {
  constructor(platform) {
    ...
    this.connectionPromise = new Promise((resolve, reject) => {
          ...
          switch (command) {
            case commands.scan: {
              ...
              if (existingIndex === -1) {
                ...
                if (!platformConsts.lanModels.includes(deviceData.sku)) {
                  this.log.warn('[LAN] %s [%s].', platformLang.supportLAN, deviceData.sku);
                }
            ...
            case commands.deviceStatus: {
              ...
              if (foundDeviceId) {
                ...
                platform.receiveUpdateLAN(foundDeviceId.device, payload.msg, deviceAddress);
              } else {
      ...
      this.receiver.on('listening', () => {
        ...
        if (this.debug) {
          this.log('[LAN] %s %s:%s.', platformLang.lanServerStarted, address, port);
        }
        resolve();
      });
  ...
  async updateDevice(accessory, params) {
    ...
    accessory.logDebug(`[LAN] ${platformLang.lanStartUpdate} [${JSON.stringify(updatedParams)}]`);
