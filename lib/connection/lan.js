  ...
  constructor(platform) {
    ...
    this.connectionPromise = new Promise((resolve, reject) => {
          ...
          switch (command) {
            ...
            case commands.deviceStatus: {
              ...
              if (foundDeviceId) {
                ...
                this.log.warn('[LAN] update received from unknown device [%s].', deviceAddress);
              }
              break;
            }
            default:
      ...
      this.receiver.on('listening', () => {
        ...
        if (this.debug) {
          this.log('[LAN] server started listening on %s:%s.', address, port);
        }
        resolve();
      });
    ...
    this.statusPolling = setInterval(async () => {
      this.lanDevices.forEach(async (device) => {
        try {
          ...
          this.log.warn('[%s] [LAN] could not request device status as %s.', device.device, parseError(err));
        }
      });
