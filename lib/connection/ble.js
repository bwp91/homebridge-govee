...
module.exports = class BluetoothLED {
    ...
    btClient.on('discover', async device => {
      try {
        ...
        this.device.on('disconnect', (reason) => {
          ...
          if (this.accessory) {
            // if (this.accessory.context.enableDebugLogging) {
            this.log('[%s] %s [%s].', this.accessory.displayName, this.lang.offlineBTConn, reason)
            // }
          } else if (platform.config.debug) {
            this.log(
              '[noble] [%s] %s [%s].',
              this.device ? this.device.address : 'unknown',
              this.lang.offlineBTConn,
              reason
            )
          }
        ...
        device.discoverAllServicesAndCharacteristics((error, services, characteristics) => {
          ...
          for (const char of characteristics) {
            ...
            if (formattedChar === '000102030405060708090a0b0c0d2b11') {
              ...
              this.log('[%s] found correct characteristic [%s].', this.accessory.displayName, formattedChar)
            } else {
              this.log('[%s] found different characteristic [%s].', this.accessory.displayName, formattedChar)
            }
          }
          if (!this.controlChar) {
  ...
  async updateDevice (accessory, params) {
    ...
    this.log('[%s] starting update with params [%s].', accessory.displayName, JSON.stringify(params))
    ...
    while (true) {
      // Timeout if taking over 4 seconds
      if (!doIt) {
