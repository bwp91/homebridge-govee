  ...
  constructor (platform) {
    ...
    btClient.on('discover', async device => {
      try {
        ...
        // If the address doesn't match and the local name doesn't match then don't continue
        if (
          this.accessory.context.bleAddress !== device.address &&
          (!this.accessory.context.bleName ||
            this.accessory.context.bleName !== device.advertisement.localName)
        ) {
          return
        }
