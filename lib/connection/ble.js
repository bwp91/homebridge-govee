  ...
  constructor(platform) {
    ...
    btClient.on('discover', (device) => {
      ...
      this.device.connect((error) => {
        if (error) {
          this.log.warn('[%s] could not connect as %s.', this.accessory.displayName, error.message);
          return;
        }
        ...
        device.discoverAllServicesAndCharacteristics((error2, services, characteristics) => {
          if (error2) {
            this.log.warn('[%s] could not find device characteristics as %s.', this.accessory.displayName, error2.message);
            return;
          }
