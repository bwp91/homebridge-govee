...
export default class {
  constructor(platform) {
    ...
    btClient.on('scanStart', () => {
      if (platform.config.debug) {
        this.log('[BLE] %s.', platformLang.btStart);
      }
    });
    ...
    btClient.on('scanStop', () => {
      if (platform.config.debug) {
        this.log('[BLE] %s.', platformLang.btStop);
      }
    });
    ...
    btClient.on('discover', (device) => {
      ...
      this.device.on('disconnect', (reason) => {
        ...
        if (this.accessory) {
          this.accessory.logDebug(`${platformLang.offlineBTConn} [${reason}]`);
  ...
  async updateDevice(accessory, params) {
    ...
    accessory.logDebug('[%s] scanning started');
    ...
    accessory.logDebug(`[BLE] ${platformLang.sendingUpdate} {${params.cmd}: ${JSON.stringify(params.data)}`);
