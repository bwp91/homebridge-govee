...
module.exports = class BluetoothLED {
  constructor (platform, accessory, device) {
    ...
    this.device.on('disconnect', () => {
      if (this.accessory.context.enableDebugLogging) {
        this.platform.log('[%s] %s.', this.accessory.displayName, this.platform.lang.offlineBT)
      }
    })
  }
  ...
  async updateDevice (params) {
    ...
    if (this.accessory.context.enableDebugLogging) {
      this.platform.log('[%s] %s.', this.accessory.displayName, this.platform.lang.onlineBT)
    }
