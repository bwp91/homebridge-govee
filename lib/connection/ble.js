...
module.exports = class BluetoothLED {
  ...
  async updateDevice (accessory, params) {
    ...
    switch (params.name) {
      case 'turn':
        await this.send(accessory, 0x01, params.value === 'on' ? 0x1 : 0x0)
        break
      case 'brightness': {
        const percent = this.platform.consts.scaleBrightness.includes(accessory.context.gvModel)
          ? Math.round(params.value / 2.54)
          : params.value
        await this.send(accessory, 0x04, Math.floor((percent / 100) * 0xff))
        break
      }
      case 'color':
        await this.send(accessory, 0x05, [0x02, params.value.r, params.value.g, params.value.b])
        break
      case 'colorTem': {
        const [r, g, b] = this.platform.colourUtils.k2rgb(params.value)
        await this.send(accessory, 0x05, [0x02, 0xff, 0xff, 0xff, 0x01, r, g, b])
        break
      }
      default:
  ...
  async send (accessory, cmd, payload) {
