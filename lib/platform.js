...
export default class {
    ...
    try {
      ...
      if (bleSyncNeeded) {
        try {
          ...
          if (!this.bleClient) {
            throw new Error('no BLE client detected');
          }
          ...
          const eText = parseError(e, ['no BLE client detected']);
          this.log.warn('BLE scanning for thermo-hygrometer sensors disabled as %s.', eText);
        }
      }
    ...
    try {
      await this.sensorStartDiscovery((goveeReading) => {
        ...
        if (accessory) {
          ...
          this.log.warn('[%s] BLE reading received for known accessory.', goveeReading.address);
        }
      });
      ...
      setTimeout(async () => {
        try {
          ...
          this.log.warn('Could not stop BLE sync as %s.', parseError(err));
        }
      }, 10000);
    } catch (err) {
      this.log.warn('Could not start BLE sync as %s.', parseError(err));
    }
  }
    ...
    if (accessory.context.useLANControl && data.lanParams) {
      try {
        ...
        if (accessory.enableLogging) {
          this.log.warn('[%s] LAN mode failed as %s.', accessory.displayName, parseError(err));
        }
      }
    }
  ...
  receiveDeviceUpdate(accessory, params) {
    ...
    if (!accessory?.control?.externalUpdate) {
      if (accessory.context.enableDebugLogging) {
        this.log.warn('[%s] no externalUpdate function to process incoming update.', accessory.displayName);
      }
      return;
    }
