      ...
      try {
        ...
        try {
          ...
          if (storedData[2] !== this.config.username) {
            ...
            throw new Error('username has changed');
          }
          ...
          this.log.warn('[HTTP] could not use existing access token as %s.', parseError(err));
          ...
          try {
            ...
            this.log.warn('[HTTP] could not store access token as %s.', parseError(e));
          }
          await getDevices();
        }
      } catch (err) {
