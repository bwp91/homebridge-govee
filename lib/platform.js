...
export default class {
  ...
  async pluginSetup() {
    ...
    try {
      ...
      this.lanQueue = new PQueue({
        concurrency: 6,
        interval: this.config.lanControlInterval,
        intervalCap: 1,
        timeout: 10000,
        throwOnTimeout: true,
      });
      ...
      if (httpDevices && httpDevices.length > 0) {
        ...
        httpDevices.forEach((httpDevice) => {
          ...
          if (lanDevice) {
            ...
            lanDevice.httpInfo = httpDevice;
            lanDevice.isLANDevice = true;

            this.initialiseDevice(lanDevice);
        ...
        lanDevices.forEach((lanDevice) => {
          ...
          if (apiDevice) {
            ...
            this.initialiseDevice({
              ...
              deviceName: lanDevice.device, // Since LAN does not provide a name, we will use the device id
              model: lanDevice.sku,
              isLANDevice: true,
            });
          }
  ...
  async sendDeviceUpdate(accessory, params) {
    ...
    // Check to see if we have the option to use LAN. If we do, we will process this request on a faster queue.
    if (
      accessory.content.useLANControl
      && platformConsts.lanModels.includes(accessory.context.gvModel)
      && data.lanParams
    ) {
      return this.lanQueue.add(async () => {
        try {
          await this.lanClient.updateDevice(accessory, data.lanParams);
        } catch (err) {
          // Errors are shown from the LAN client instead
        }
      });
    }

    // At the moment if LAN fails we do not continue with the other methods.
  ...
  receiveDeviceUpdateForId(accessoryId, params) {
    devicesInHB.forEach(async (accessory) => {
