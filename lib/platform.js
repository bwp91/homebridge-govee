...
export default class {
                        ...
                        iotRule.rule.forEach((rule) => {
                          if (rule.iotMsg) {
                            ...
                            if (iotMsg.msg?.cmd === 'ptReal') {
                              this.log('[%s] [%s] %s', iotRule.deviceObj.name, oneClick.name, iotMsg.msg.data.command.join(','));
                            }
                          }
                        });
                      }
  ...
  async sendDeviceUpdate(accessory, params) {
    ...
    switch (params.cmd) {
      ...
      case 'rgbScene': {
        // We get the `ptReal` scene code in a string format, commands separated by a comma
        data.awsParams = {
          cmd: 'ptReal',
          data: {
