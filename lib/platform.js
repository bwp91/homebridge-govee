    ...
    try {
      ...
      try {
        ...
        try {
          ...
          [this.accountTopic, this.accountToken] = storedData.split(':::');
          ...
          try {
            await this.storageData.setItem('Govee_All_Devices_temp', `${this.accountTopic}:::${accountToken}`);
          } catch (e) {
      ...
      try {
        ...
        this.log('[API] %s.', platformLang.availableWithDevice(devices.length));
      } catch (err) {
      ...
      if (bleSyncNeeded) {
        try {
          ...
          this.log.warn('[BLE] %s %s.', platformLang.btScanDisabled, parseError(err, [platformLang.btNoPackage]));
        }
      }
    ...
    try {
      await this.sensorStartDiscovery((goveeReading) => {
        ...
        if (accessory) {
          ...
          this.log.warn('[BLE] %s [%s].', platformLang.btScanUnknown, goveeReading.address);
        }
      });
      ...
      setTimeout(async () => {
        try {
          ...
          this.log.warn('[BLE] %s %s.', platformLang.btScanNoStop, parseError(err));
        }
      }, 5000);
    } catch (err) {
      this.log.warn('[BLE] %s %s.', platformLang.btScanNoStart, parseError(err));
    }
  }
