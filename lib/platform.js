...
export default class {
    ...
    try {
      ...
      try {
        ...
        if (this.config.debug) {
          this.log.warn('%s %s.', platformLang.storageSetupErr, parseError(err));
        }
      }
      ...
      try {
        if (this.config.lanDisable) {
          throw new Error('disabled in config');
        }
    ...
    try {
      ...
      if (platformConsts.models.rgb.includes(device.model)) {
        ...
        if (this.bleClient) {
          ...
          this.log('[%s] [%s] %s.', device.deviceName, device.device, platformLang.btNoPackage);
          return;
        }
  ...
  async goveeHTTPSync() {
    try {
      ...
      this.log.warn('%s %s.', platformLang.httpSyncFail, parseError(err));
    }
  }
    ...
    devicesInHB.forEach(async (accessory) => {
      try {
        ...
        if (err.message.includes('400')) {
          accessory.logDebug(platformLang.devNotRet);
          return;
        }
    ...
    return this.queue.add(async () => {
      ...
      if (accessory.context.useBLEControl && data.bleParams) {
        try {
          ...
          accessory.logDebugWarn(`${platformLang.notBLESent} ${parseError(err, [platformLang.btTimeout])}`);
        }
      }
  ...
  receiveDeviceUpdate(accessory, params) {
    ...
    if (!accessory?.control?.externalUpdate) {
      accessory.logDebugWarn(platformLang.noExtFunc);
      return;
    }
