  ...
  async pluginSetup() {
    ...
    try {
      ...
      try {
        ...
        this.log('%s.', platformLang.availableLAN);
      } catch (err) {
        const eText = parseError(err);
        this.log.warn('%s %s.', platformLang.disableLAN, eText);
        this.lanClient = false;
      }
      ...
      if (httpDevices && httpDevices.length > 0) {
        ...
        httpDevices.forEach((httpDevice) => {
          ...
          // Find any matching device from the LAN and API client
      ...
      if (bleSyncNeeded) {
        try {
          ...
          const eText = parseError(e, [platformLang.btNoPackage]);
          this.log.warn('%s %s.', platformLang.btScanDisabled, eText);
        }
      }
