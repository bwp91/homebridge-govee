...
export default class {
  ...
  async pluginSetup() {
    ...
    try {
      ...
      if (lanDevices && lanDevices.length > 0) {
        lanDevices.forEach((lanDevice) => {
          ...
          this.initialiseDevice({
            ...
            // Since LAN does not provide a name, we will use the device id
            deviceName: lanDevice.device,
            model: lanDevice.sku,
            isLANDevice: true,
          });
  ...
  async sendDeviceUpdate(accessory, params) {
    ...
    const commandResult = await new Promise((resolve) => {
      ...
      if (platformConsts.lanModels.includes(accessory.context.gvModel) && data.lanParams) {
        this.lanQueue.add(async () => {
          resolve(await this.lanClient.updateDevice(accessory, data.lanParams));
        });
      } else {
        resolve(false);
      }
    ...
    return this.queue.add(async () => {
      try {
        ...
        // if (platformConsts.lanModels.includes(accessory.context.gvModel) && data.lanParams) {
        //   const result = await this.lanClient.updateDevice(accessory, data.lanParams);
        //   if (result === true) {
        //     return;
        //   }
        // }
