...
export default class {
        ...
        if (device.httpInfo.deviceExt && device.httpInfo.deviceExt.deviceSettings) {
          ...
          if (parsed && parsed.topic) {
            ...
            if (this.awsClient) {
              ...
              if (platformConsts.awsModelsToPoll.includes(device.model)) {
  ...
  receiveUpdateAWS(payload) {
    ...
    const toSend = {
      source: 'AWS',
      ...payload.state, // backward compatibility
      op: payload.op, // backward compatibility
      bulb: payload.cmd === 'bulb' && Array.isArray(payload.state.value) // backward compatibility
        ? payload.state.value
        : undefined,
      ...payload,
    };
  ...
  receiveDeviceUpdate(accessory, params) {
    ...
    */
    const colourParams = params.data || params;
    if (params.color) {
      data.rgb = params.color;
    } else if (params.cmd === 'color') {
      if (hasProperty(colourParams, 'red')) {
        data.rgb = {
          r: colourParams.red,
          g: colourParams.green,
          b: colourParams.blue,
        };
      } else if (hasProperty(colourParams, 'r')) {
        data.rgb = {
          r: colourParams.r,
          g: colourParams.g,
          b: colourParams.b,
    ...
    } else if (params.cmd === 'colorwc' && hasProperty(colourParams.color, 'red')) {
      data.rgb = {
        r: colourParams.color.red,
        g: colourParams.color.green,
        b: colourParams.color.blue,
      };
    } else if (params.cmd === 'colorwc' && hasProperty(colourParams.color, 'r')) {
      data.rgb = {
        r: colourParams.color.r,
        g: colourParams.color.g,
        b: colourParams.color.b,
      };
    }
    ...
    */
    if (params.colorTemInKelvin) {
      ...
      data.kelvin = colourParams.colorTemInKelvin;
    } else if (params.cmd === 'colorwc' && colourParams.colorTemInKelvin > 0) {
      data.kelvin = colourParams.colorTemInKelvin;
    }
