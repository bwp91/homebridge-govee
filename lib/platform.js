...
export default class {
  ...
  async pluginSetup() {
    ...
    try {
      ...
      try {
        ...
        this.log('%s.', platformLang.availableHTTP(devices.length));
    ...
    try {
      awsDevices.forEach(async (deviceId) => {
        ...
        try {
          ...
          accessory.logDebugWarn(`${platformLang.syncFailAWS} ${parseError(err)}`);
        }
      });
    } catch (err) {
      this.log.warn('%s %s.', platformLang.syncFailAWS, parseError(err));
    }
  }
  ...
  async goveeHTTPSync() {
    try {
      ...
      this.log.warn('%s %s.', platformLang.syncFailHTTP, parseError(err));
    }
  }
