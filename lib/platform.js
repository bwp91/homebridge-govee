...
export default class {
  ...
  async pluginSetup() {
    ...
    try {
      ...
      try {
        ...
        const iotFileData = await PfxToPem.toPem({ path: iotFile, password: this.iotPass });
        ...
        if (err.message.includes('Command failed')) {
          this.log.warn(platformLang.wrongOpenSSL);
        }
      ...
      this.log.warn('***** %s [%s]. *****', platformLang.disabling, plugin.version);
    ...
    switch (params.cmd) {
      ...
      case 'scene': {
        ...
        */
        ...
        // For BLE, we need to map each two digits to a hex value and also remove the first two items
        const awsCode = Buffer.from(params.value, 'base64').toString('hex');
        const bleCode = hexToTwoItems(params.value.substring(4)).map((item) => `0x${item}`);
  ...
  receiveUpdateAWS(payload) {
    ...
    if (payload.op && Array.isArray(payload.op.command)) {
      toSend.commands = payload.op.command;
    }
  ...
  receiveDeviceUpdate(accessory, params) {
    ...
    */
    ...
    } else if (params.op?.commands) {
      data.commands = params.op.commands;
    }
