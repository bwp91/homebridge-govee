    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (storedData[2] !== this.config.username) {
      ...
      if (device.httpInfo) {
        ...
        if (device.httpInfo.deviceExt && device.httpInfo.deviceExt.deviceSettings) {
          ...
          if (parsed && parsed.topic) {
            ...
            accessory.context.awsStatusCode = platformConsts.awsStatusZero.includes(device.device)
              ? [0, 'y']
              : [2, 'v'];
            awsDevices.push(device.device);
            if (deviceConf.awsDebug) {
              accessory.awsDebug = new awsDeviceClient(this, accessory);
            }
          }
    ...
    try {
      await this.sensorStartDiscovery((goveeReading) => {
        ...
        if (accessory) {
          ...
          this.log.debugWarn('[BLE] %s [%s].', platformLang.btScanUnknown, goveeReading.address);
        }
      });
