...
export default class {
  ...
  initialiseDevice(device) {
    ...
    try {
      ...
      accessory.context.awsDebug = deviceConf.awsDebug || false;
      ...
      if (device.httpInfo) {
        ...
        if (device.httpInfo.deviceExt && device.httpInfo.deviceExt.deviceSettings) {
          ...
          if (parsed && parsed.topic) {
            ...
            if (accessory.context.awsDebug || needsClient || ['H7101', 'H7111'].includes(device.model)) {
    ...
    devicesInHB.forEach((accessory) => {
      if (accessory.context.gvDeviceId === accessoryId) {
        ...
        if (!accessory.context.useLANControl) {
          accessory.context.useLANControl = true;
          accessory.log('LAN control enabled');
          update = true;
        }
        ...
        if (accessory.context.ipAddress !== ipAddress) {
          ...
          accessory.log(`current IP ${ipAddress}`);
          update = true;
        }
  ...
  receiveDeviceUpdate(accessory, params) {
    ...
    if (!accessory?.control?.externalUpdate || Object.keys(params).length === 0) {
      return;
    }
