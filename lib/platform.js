...
export default class {
  ...
  async pluginSetup() {
    ...
    try {
      ...
      try {
        ...
        this.log('%s.', platformLang.availableLAN(devices.length));
      } catch (err) {
        this.log.warn('%s %s.', platformLang.disableLAN, parseError(err));
        this.lanClient = false;
      }
      ...
      try {
        ...
        this.log('%s.', platformLang.availableAWS(devices.length));
      } catch (err) {
        this.log.warn('%s %s.', platformLang.disableHTTP, parseError(err, [platformLang.noCreds]));
        this.httpClient = false;
      }
      ...
      try {
        ...
        this.log('%s.', platformLang.availableAPI(devices.length));
      } catch (err) {
        const eText = err.message.includes('401')
          ? platformLang.invalidApiKey
          : parseError(err, [platformLang.noAPIKey]);
      ...
      try {
        ...
        */
        ...
        this.log.warn('%s %s.', platformLang.disableBT, parseError(err, [
      ...
      if (httpDevices && httpDevices.length > 0) {
      ...
      } else if (lanDevices && lanDevices.length > 0) {
        ...
        lanDevices.forEach((lanDevice) => {
          ...
          // Find any matching device from the API client
          const apiDevice = apiDevices.find((el) => el.device === lanDevice.device);
      ...
      if (bleSyncNeeded) {
        try {
          ...
          // TODO let the user configure this refresh interval
          this.refreshBLEInterval = setInterval(
            () => this.goveeBLESync(),
            120000,
          );
        } catch (err) {
          this.log.warn('%s %s.', platformLang.btScanDisabled, parseError(err, [platformLang.btNoPackage]));
        }
      }
      ...
      if (apiDevicesWereInitialised) {
        ...
        // TODO once appliance commands are implemented, we don't need to filter out these appliances
        const apiArray = accessoryArray.filter((el) => el.context.isAPILight);
        ...
        if (apiCount > 0) {
          ...
          if (pollingPeriod > this.config.refreshTime) {
            if (this.config.apiWhitelist) {
              this.log.warn('%s.', platformLang.apiPollingWhitelist);
            } else {
              this.config.refreshTime = pollingPeriod;
              this.log.warn('%s %ss .', platformLang.apiPollingIncrease, this.config.refreshTime);
            }
          }
          this.goveeAPISync();
  ...
  initialiseDevice(device) {
    ...
    try {
      ...
      if (platformConsts.models.rgb.includes(device.model)) {
        ...
        // Device is a heater
        devInstance = deviceConf.temperatureSource
          ? deviceTypes.deviceHeaterTemp
          : deviceTypes.deviceHeater;
        accessory = devicesInHB.get(uuid) || this.addAccessory(device);
      ...
      accessory.context.isAPILight = device.isAPIDevice && platformConsts.models.rgb.includes(device.model);
    ...
    try {
      await this.sensorStartDiscovery((goveeReading) => {
        ...
        if (accessory) {
          ...
          this.log.warn('[%s] %s.', goveeReading.address, platformLang.btScanUnknown);
        }
      });
      ...
      setTimeout(async () => {
        try {
          ...
          this.log.warn('%s %s.', platformLang.btScanNoStop, parseError(err));
        }
      }, 5000);
    } catch (err) {
      this.log.warn('%s %s.', platformLang.btScanNoStart, parseError(err));
    }
  }
            ...
            if (platformConsts.models.leak.includes(device.sku)) {
              if (accessory.context.enableDebugLogging) {
                this.log(
                  '[%s] raw data: %s.',
                  device.deviceName,
                  JSON.stringify({ ...parsedData, ...parsedSettings }),
                );
              }
  ...
  async sendDeviceUpdate(accessory, params) {
    ...
    if (accessory.context.useLANControl && data.lanParams) {
      try {
        ...
        if (accessory.enableLogging) {
          this.log.warn(
            '[%s] %s %s.',
            accessory.displayName,
            platformLang.notLANSent,
            parseError(err, [platformLang.lanDevNotFound]),
          );
        }
      }
    }
    ...
    if (accessory.context.useAWSControl && data.awsParams) {
      try {
        await accessory.awsControl.updateDevice(data.awsParams);
        return true;
      } catch (err) {
        ...
        if (accessory.context.enableDebugLogging) {
          this.log.warn(
            '[%s] %s %s.',
            accessory.displayName,
            platformLang.notAWSSent,
            parseError(err, [platformLang.notAWSConn]),
          );
        }
      }
    }
        ...
        try {
          ...
          if (accessory.context.enableDebugLogging) {
            this.log.warn(
              '[%s] %s %s.',
              accessory.displayName,
              platformLang.notBLESent,
              parseError(err, [platformLang.btTimeout]),
            );
          }
        }
      }
  ...
  receiveDeviceUpdateForId(accessoryId, params, ipAddress) {
    devicesInHB.forEach(async (accessory) => {
  ...
  receiveDeviceUpdate(accessory, params) {
    ...
    if (!accessory?.control?.externalUpdate) {
      if (accessory.context.enableDebugLogging) {
        this.log.warn('[%s] %s.', accessory.displayName, platformLang.noExtFunc);
      }
      return;
    }
