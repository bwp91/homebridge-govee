...
export default class {
          ...
          try {
            ...
            if (platformConsts.models.leak.includes(device.sku)) {
              ...
              if (parsedData.lastTime > 0) {
                ...
                const unreadCount = msgs.filter((msg) => !msg.read && msg.message.toLowerCase().startsWith('leakage alert'));
  ...
  async sendDeviceUpdate(accessory, params) {
    ...
    if (accessory.context.useAWSControl && data.awsParams) {
      try {
        accessory.awsControl.updateDevice(data.awsParams); // no response from this
        await sleep(250);
      } catch (err) {
