...
import deviceCoolerSingle from './device/cooler-single.js';
import deviceFan from './device/fan.js';
import deviceHeaterSingle from './device/heater-single.js';
import deviceHeater from './device/heater.js';
import deviceHumidifier from './device/humidifier.js';
import deviceLightColourBt from './device/light-colour-bt.js';
import deviceLightColour from './device/light-colour.js';
import deviceOutletDouble from './device/outlet-double.js';
import deviceOutletSingle from './device/outlet-single.js';
import devicePurifierSingle from './device/purifier-single.js';
import devicePurifier from './device/purifier.js';
import deviceSensorLeak from './device/sensor-leak.js';
import deviceSensorThermo from './device/sensor-thermo.js';
import deviceSwitchDouble from './device/switch-double.js';
import deviceSwitchSingle from './device/switch-single.js';
import deviceTapSingle from './device/tap-single.js';
import deviceTVSingle from './device/tv-single.js';
import deviceValveSingle from './device/valve-single.js';
import eveService from './fakegato/fakegato-history.js';
import { k2rgb } from './utils/colour-utils.js';
...
export default class {
        ...
        case 'thermoDevices':
          if (Array.isArray(val) && val.length > 0) {
            val.forEach((x) => {
              ...
              const id = platformFuncs.parseDeviceId(x.deviceId);
    ...
    try {
      ...
      try {
        this.storageData = storage.create({
          dir: path.join(this.api.user.persistPath(), '/../bwp91_cache'),
      ...
      try {
        ...
        const eText = platformFuncs.parseError(err, [platformLang.noCreds, platformLang.noDevList]);
      ...
      try {
        ...
        const eText = err.message.includes('401')
          ...
          : platformFuncs.parseError(err, [platformLang.noAPIKey, platformLang.noDevList]);
      ...
      try {
        ...
        */
        ...
        this.log('%s.', platformLang.btAvailable);
      } catch (err) {
      ...
      const eText = platformFuncs.parseError(err, [platformLang.noDevs, platformLang.disabled]);
    ...
    try {
      ...
      if (platformConsts.models.rgb.includes(device.model)) {
        ...
        if (this.bleClient) {
          ...
          isLight = true;
          devInstance = deviceLightColourBt;
          accessory = devicesInHB.get(uuid) || this.addAccessory(device);
        } else {
        ...
        switch (deviceConf.showAs || platformConsts.defaultValues.showAs) {
          ...
          case 'cooler': {
            ...
            devInstance = deviceCoolerSingle;
            accessory = devicesInHB.get(uuid) || this.addAccessory(device);
            break;
          }
          case 'heater': {
            ...
            devInstance = deviceHeaterSingle;
            accessory = devicesInHB.get(uuid) || this.addAccessory(device);
            break;
          }
          case 'purifier': {
            devInstance = devicePurifierSingle;
            accessory = devicesInHB.get(uuid) || this.addAccessory(device);
            break;
          }
          case 'switch': {
            devInstance = deviceSwitchSingle;
            accessory = devicesInHB.get(uuid) || this.addAccessory(device);
            break;
          }
          case 'tap': {
            devInstance = deviceTapSingle;
            accessory = devicesInHB.get(uuid) || this.addAccessory(device);
            break;
          }
          case 'valve': {
            devInstance = deviceValveSingle;
            accessory = devicesInHB.get(uuid) || this.addAccessory(device);
            break;
          }
          default:
            devInstance = deviceOutletSingle;
            accessory = devicesInHB.get(uuid) || this.addAccessory(device);
            break;
        }
        ...
        switch (deviceConf.showAs || platformConsts.defaultValues.showAs) {
          case 'switch': {
            devInstance = deviceSwitchDouble;
            accessory = devicesInHB.get(uuid) || this.addAccessory(device);
            break;
          }
          default: {
            devInstance = deviceOutletDouble;
            accessory = devicesInHB.get(uuid) || this.addAccessory(device);
            break;
          }
        }
      ...
      if (device.httpInfo) {
        ...
        if (device.httpInfo.deviceExt && device.httpInfo.deviceExt.deviceSettings) {
          ...
          if (parsed && platformFuncs.hasProperty(parsed, 'temMin') && parsed.temMax) {
          ...
          if (parsed && platformFuncs.hasProperty(parsed, 'humMin') && parsed.humMax) {
  ...
  async goveeAPISync() {
    devicesInHB.forEach(async (accessory) => {
      try {
        ...
        let eText;
        if (platformFuncs.isGoveeError(err)) {
          ...
          eText = platformFuncs.parseError(err);
        }
        if (accessory.context.enableDebugLogging) {
  ...
  async sendDeviceUpdate(accessory, params) {
    ...
    try {
      ...
      /*
        ...
        BLE devices: we try three before finally returning an error to HomeKit
      */
      // try {
      await this.bleClient.updateDevice(accessory, data.bleParams);
      // } catch (err) {
      //   // Try a couple more times for ble only models
      //   if (!accessory.context.hasAPIControl) {
      //     try {
      //       await this.bleClient.updateDevice(accessory, data.bleParams)
      //     } catch (err) {
      ...
      if (accessory.context.useAPIControl && data.apiParams) {
        ...
        if (accessory.context.enableDebugLogging && accessory.context.useBLEControl) {
          const eText = platformFuncs.parseError(err, [platformLang.btTimeout]);
  ...
  receiveDeviceUpdate(accessory, params) {
    ...
    */
    ...
    if (platformFuncs.hasProperty(params, 'online')) {
    ...
    */
    if (platformFuncs.hasProperty(params, 'brightness')) {
    ...
    */
    if (params.color) {
      ...
      if (platformFuncs.hasProperty(params.data, 'red')) {
        data.rgb = {
          r: params.data.red,
          g: params.data.green,
          b: params.data.blue,
        };
      } else if (platformFuncs.hasProperty(params.data, 'r')) {
        data.rgb = {
          r: params.data.r,
          g: params.data.g,
          b: params.data.b,
        ...
        if (
          !platformConsts.awsColourShort.includes(accessory.context.gvModel)
          && !platformFuncs.hasProperty(params.data, 'red')
        ) {
          this.log.warn(
            '[%s] %s [%s].',
            accessory.displayName,
    ...
    } else if (params.cmd === 'colorwc' && platformFuncs.hasProperty(params.data.color, 'red')) {
      data.rgb = {
    ...
    */
    if (platformFuncs.hasProperty(params, 'battery')) {
    ...
    */
    if (platformFuncs.hasProperty(params, 'leakDetected')) {
