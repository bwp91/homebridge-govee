  ...
  async pluginSetup() {
    ...
    try {
      ...
      try {
        ...
        const iotFile = join(this.api.user.persistPath(), 'govee.pfx');
        ...
        try {
          ...
          try {
            await fsPromises.access(iotFile, 0);
          } catch (err) {
            throw new Error('IoT certificate does not exist');
          }
          ...
          // Save this to a file
          await fsPromises.writeFile(iotFile, Buffer.from(data.iot, 'base64'));
      ...
      if (device.httpInfo) {
        ...
        if (device.httpInfo.deviceExt && device.httpInfo.deviceExt.deviceSettings) {
          ...
          if (parsed && parsed.topic) {
            ...
            accessory.context.awsStatusCode = [2, 'v'];
