    ...
    try {
      ...
      if (!this.devicesInHB.has(device.device)) {
        if (!cns.supportedModels.includes(device.model)) {
          ...
          if (!this.config.allowNonSupported) {
            this.log.warn('[%s] ...but will try to be added anyway as per configuration...', device.deviceName)
            this.log.warn('[%s] ...and if it causes issues then please revert the setting.', device.deviceName)
          } else {
            this.log.warn('[%s] ...but might work anyway by enabling "Allow Non-Supported Devices"...', device.deviceName, device.model)
            this.log.warn('[%s] ...but attempting this is at your own risk!', device.deviceName, device.model)
            return
          }
        }
