    ...
    try {
      ...
      if (cns.modelsLED.includes(device.model)) {
        ...
        *********/
        ...
        accessory.control = new DeviceLED(this, accessory)
        /********/
      } else {
        /**********
        UNSUPPORTED
        **********/
        this.log.warn(' → [%s] has not been added as it is not supported.', device.name)
        return
        /*********/
      }
      ...
      this.log(' → [%s] initialised from your Govee account.', accessory.displayName)
    } catch (err) {
      if (this.debug) {
