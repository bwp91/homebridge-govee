  ...
  async goveeSetup () {
    try {
      ...
      this.refreshStatus = promInterval(
        ...
        (parseInt(this.config.refreshTime) || cns.refreshTime) * 1000,
        { stopOnError: false }
      )
  ...
  async goveeStatusSync () {
    this.devicesInHB.forEach(async accessory => {
      try {
        ...
        accessory.control.externalUpdate(accessory, Object.assign({}, ...res))
      } catch (err) {
    ...
    try {
      ...
      if (cns.modelsLED.includes(device.model)) {
        ...
        *********/
        ...
        if (!utils.hasProperty(accessory, 'control')) accessory.control = new DeviceLED(this, accessory)
        /********/
      } else {
        /**********
        UNSUPPORTED
        **********/
        if (!this.firstRunDone) this.log.warn(' → [%s] has not been added as it is not supported.', device.name)
        return
        /*********/
      }
      ...
      if (!this.firstRunDone) this.log(' → [%s] initialised from your Govee account.', accessory.displayName)
    } catch (err) {
      if (this.debug) {
        this.log.warn('[%s] could not be initialised as %s.', device.deviceName, err)
      }
    }
  }
