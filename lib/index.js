      ...
      if (httpDevices && httpDevices.length > 0) {
        httpDevices.forEach(device => {
          ...
          if (apiDevice) {
            ...
            const btDevice = this.deviceConf.find(el => el.deviceId === device.device)
            if (btDevice) {
              btDevice.model = device.sku
              btDevice.httpInfo = device
              this.initialiseDevice(btDevice)
            } else {
              this.log('[%s] %s.', device.deviceName, this.lang.notSupported)
            }
          }
        })
  ...
  initialiseDevice (device) {
    ...
    try {
      ...
      if (this.consts.models.rgb.includes(device.model)) {
        ...
        this.log.warn('[%s] %s.', device.deviceName, this.lang.notSupported)
        return
      }
