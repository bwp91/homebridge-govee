...
class GoveePlatform {
  ...
  async sendDeviceUpdate (accessory, params) {
    ...
    try {
      ...
      // If the only connection method is bluetooth we need to throw the error again
      if (!accessory.context.useAPIControl && !accessory.context.useBLEControl) {
        throw new Error(this.lang.notAvailable)
      }

      // API doesn't support dual outlets and scenes yet so return now
      if (['stateDual', 'scene'].includes(params.cmd) && data.awsParams) {
        return
      }
