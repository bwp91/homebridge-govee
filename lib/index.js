...
class GoveePlatform {
  ...
  addLinkedAccessory (masterAccessory, linkedDevice) {
    try {
      ...
      const accessory = new this.api.platformAccessory(linkedDevice.name, uuid)
      accessory
      ...
      this.log('[%s] %s.', linkedDevice.name, this.lang.devAdd)
      return accessory
    } catch (err) {
      // Catch any errors during add
      const eText = this.funcs.parseError(err)
      this.log.warn('[%s] %s %s.', name, this.lang.devNotAdd, eText)
    }
  }
