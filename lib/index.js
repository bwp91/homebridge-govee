...
class GoveePlatform {
      ...
      if (device.httpInfo) {
        ...
        if (device.httpInfo.deviceExt && device.httpInfo.deviceExt.deviceSettings) {
          ...
          if (parsed && parsed.topic) {
            ...
            if (deviceConf && !deviceConf.disableAWS) {
          ...
          case 'scene': {
            ...
            */
            if (params.value.charAt() === '0') {
              ...
              // It seems some (newer?) models use a different payload format
              if (this.consts.reformatPayload.includes(accessory.context.gvModel)) {
                data.awsParams = {
                  cmd: 'ptReal',
                  data: {
  ...
  receiveDeviceUpdate (accessory, params) {
    ...
    */
    ...
    // Some models use a different payload format
    if (params.cmd === 'ptReal') {
      data.scene2 = params.data.command.join(',')
    }
