...
class GoveePlatform {
  ...
  async goveeSetup () {
    try {
      ...
      this.log.warn('⚠️ If you are having issues with brightness for your Govee devices, please create an issue on GitHub with your model number!')
    } catch (err) {
      this.log.warn('*** Disabling plugin [v%s] ***', this.version)
      this.log.warn(
    ...
    try {
        ...
        try {
          ...
          if (this.helpers.modelsLED.includes(device.model)) {
            ...
            *********/
            ...
            if (!accessory.control) accessory.control = new (require('./device/led-strip'))(this, accessory)
            /********/
          } else {
            /**********
            UNSUPPORTED
          ...
          if (this.debug) this.log.warn('[%s] failed to initialise as %s.', device.deviceName, err)
        }
      })
      this.devicesInHB.forEach(async a => {
        ...
        try {
          ...
          if (this.debug) this.log.warn('[%s] failed to update as as %s.', a.displayName, err.message)
        }
      })
    } catch (err) {
