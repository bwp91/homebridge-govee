...
class GoveePlatform {
  ...
  async pluginSetup () {
    ...
    try {
      ...
      if (btClient && Object.keys(bleDevices).length > 0) {
        ...
        btClient.on('warning', message => this.log.warn('[noble] %s.', message))
        btClient.on('discover', async device => {
          ...
          if (!accessory.context.onlineBT && accessory.context.enableLogging) {
        ...
        setTimeout(() => this.goveeBTSync(), 5000)
        setTimeout(() => this.goveeBTSync(), 15000)
        setTimeout(() => this.goveeBTSync(), 30000)
        this.refreshBTInterval = setInterval(() => this.goveeBTSync(), 300000)
      } else {
  ...
  pluginShutdown () {
    ...
    try {
      ...
      if (this.refreshBTInterval) {
        clearInterval(this.refreshBTInterval)
      }
    } catch (err) {
  ...
  async sendDeviceUpdate (accessory, params) {
    ...
    return await this.queue.add(async () => {
      try {
        if (accessory.btControl && btClientState === 'poweredOn') {
          // Try sending the command over bluetooth
          const timeout = accessory.context.isAPIDevice ? 2000 : 9000
          await pTimeout(accessory.btControl.updateDevice(params), timeout, this.lang.btTimeout)
        } else {
