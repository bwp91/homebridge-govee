  ...
  async pluginSetup () {
    ...
    try {
      ...
      try {
        ...
        this.log.error(this.accountTopic)
      } catch (err) {
  ...
  async sendDeviceUpdate (accessory, params) {
    ...
    return await this.queue.add(async () => {
      try {
        ...
        const eText = this.funcs.parseError(err)
        if (accessory.context.isAPIDevice) {
          // If we have reverted from bluetooth control then add a log entry showing the reason
          if (accessory.context.btControl && accessory.context.enableLogging) {
            this.log('[%s] %s %s.', accessory.displayName, this.lang.notBTSent, eText)
          }

          // Send the update via API client
