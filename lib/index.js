...
class GoveePlatform {
  constructor (log, config, api) {
    ...
    try {
      ...
      // Create variables usable by the plugin
      this.log = log
      this.api = api
  ...
  async pluginSetup () {
    ...
    try {
      // If the user has disabled the plugin then remove all accessorues
  ...
  async goveeSync () {
    try {
      ...
      if (!deviceList) {
        throw new Error('could not retrieve devices from Govee')
      }
  ...
  configureAccessory (accessory) {
    ...
    try {
      ...
      // Catch any errors during retireve
      const eText = this.funcs.parseError(err)
