...
class GoveePlatform {
  ...
  async pluginSetup () {
    ...
    try {
      ...
      this.cusServ = new (require('./utils/custom-services'))(this.api, this.scenes.rgbicAWSScenes)
  ...
  addLinkedAccessory (device, name) {
    try {
      ...
      accessory
          ...
          .setCharacteristic(this.api.hap.Characteristic.Identify, false)
      accessory.context.gvDeviceId = device.device
      accessory.context.gvModel = device.model
      this.api.registerPlatformAccessories(plugin.name, plugin.alias, [accessory])
      this.log('[%s] %s.', name, this.lang.devAdd)
      return accessory
    } catch (err) {
