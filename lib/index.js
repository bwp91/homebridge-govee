...
class GoveePlatform {
  ...
  async pluginSetup () {
    ...
    try {
      ...
      if (httpDevices && httpDevices.length > 0) {
        ...
        httpDevices.forEach(httpDevice => {
          ...
          if (apiDevice) {
            ...
            if (this.consts.models.leak.includes(httpDevice.sku)) {
              // Could be a leak sensor
              this.initialiseDevice(deviceTemplate)
            } else if (this.funcs.hasProperty(this.deviceConf, httpDevice.device)) {
              // Could be a bluetooth-only light model that's been configured manually
              if (this.deviceConf[httpDevice.device].enableBT) {
                this.initialiseDevice(deviceTemplate)
              } else {
                this.log(
                  '[%s] [%s] %s.',
                  ...
                  httpDevice.device,
                  this.lang.btNotEnabled
                )
              }
            } else {
              // Device isn't in API data, not a leak sensor, and hasn't been configured
              this.log(
                '[%s] [%s] %s.',
                httpDevice.deviceName,
                httpDevice.device,
      ...
      // Setup the http client sync needed for leak sensor devices
      if (httpSyncNeeded) {
        this.goveeAPISync()
      ...
      const eText = [this.lang.noDevs, this.lang.disabled].includes(err.message)
        ? err.message
        : this.funcs.parseError(err)
      ...
      if (device.httpInfo) {
        ...
        if (device.httpInfo.deviceExt && device.httpInfo.deviceExt.deviceSettings) {
          ...
          if (parsed && parsed.bleName) {
            ...
            if (deviceConf && deviceConf.enableBT && bleClient) {
  ...
  async goveeHTTPSync () {
    try {
      ...
      devices
        .filter(device => this.consts.models.leak.includes(device.sku))
        .forEach(async device => {
          try {
            ...
            // Check to see of any warnings if the lastTime is above 0
            ...
            this.receiveDeviceUpdate(accessory, {
              battery: parsedSettings.battery,
              hasUnreadLeak,
              online: parsedData.gwonline && parsedData.online,
              source: 'HTTP'
            })
          } catch (err) {
            const eText = this.funcs.parseError(err)
      ...
      const eText = this.funcs.parseError()
      this.log.warn('Govee HTTP Sync failed as %s.', eText)
    }
  }
        ...
        switch (params.cmd) {
          ...
          case 'colorTem': {
            /*
              ...
              AWS plugin doesn't support this yet
  ...
  receiveDeviceUpdate (accessory, params) {
    ...
    */
    if (this.funcs.hasProperty(params, 'hasUnreadLeak')) {
      data.leakDetected = params.hasUnreadLeak
    }
