              ...
              for (const [k, v] of Object.entries(x)) {
                switch (k) {
                  ...
                  case 'overrideDisabledLogging':
                    ...
                    this.switchDevices[deviceId][k] = v === 'false' ? false : !!v
                    break
                  default:
                    logRemove(key + '.' + k)
                    break
    ...
    try {
      ...
      this.devicesInHB.forEach(async accessory => {
        try {
          ...
          const name = accessory.displayName
          const eText = e.message.includes('502')
            ? this.messages.govee502
            : this.funcs.parseError(e)
          this.log.warn('[%s] %s %s.', name, this.messages.devNotRef, eText)
        }
      })
    } catch (err) {
      // Catch any errors during sync process
      const eText = err.message.includes('502')
        ? this.messages.govee502
        : this.funcs.parseError(err)
