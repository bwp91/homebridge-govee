...
class GoveePlatform {
  constructor (log, config, api) {
    ...
    try {
      ...
      this.scenes = require('./utils/scenes')
      ...
      this.getDeviceByUUID = function (UUID) {
        return devicesInHB.get(UUID) || null
      }
            ...
            val.forEach(x => {
              ...
              for (const [k, v] of entries) {
                ...
                switch (k) {
                  ...
                  case 'chill':
                  case 'aurora_custom':
                      if (!this.deviceConf[id]['lightEffects']) {
                          this.deviceConf[id]['lightEffects'] = {}
                      }
                      this.deviceConf[id]['lightEffects'][k] = !!v
                      break
                  case 'customAddress':
                  case 'diyMode':
                  case 'diyModeTwo':
      ...
      devicesInHB.forEach(async accessory => {
        ...
        if (
          (!httpDevices.some(el => el.device === accessory.context.gvDeviceId) &&
            ...
            !accessory.context.linkedDevice) ||
          ...
          if (parsed && parsed.topic) {
            ...
            try {
              if (!deviceConf.disableAWS) {
                const iotData = await httpClient.getIotData();
                accessory.context.useAWSControl = true
                accessory.awsControl = new (require('./connection/aws'))(this, accessory, iotData)
              }
            } catch (err) {
              this.log.err(err);
            }
  ...
  addLinkedAccessory(masterAccessory, linkedDevice) {
    try {
        const uuid = this.api.hap.uuid.generate(linkedDevice.id)
        const accessory = new this.api.platformAccessory(linkedDevice.deviceName, uuid)
        accessory
            .getService(this.api.hap.Service.AccessoryInformation)
            .setCharacteristic(this.api.hap.Characteristic.Manufacturer, this.lang.brand)
            .setCharacteristic(this.api.hap.Characteristic.SerialNumber, linkedDevice.id)
            .setCharacteristic(this.api.hap.Characteristic.Model, linkedDevice.model)
            .setCharacteristic(this.api.hap.Characteristic.Identify, true)
        accessory.context.gvDeviceId = linkedDevice.id
        accessory.context.gvModel = linkedDevice.model
        this.api.registerPlatformAccessories(plugin.name, plugin.alias, [accessory])
        this.configureAccessory(accessory)
        masterAccessory.addBridgedAccessory(accessory)
        this.log('[%s] %s.', linkedDevice.deviceName, this.lang.devAdd)
        return accessory
    } catch (err) {
        // Catch any errors during add
        const eText = this.funcs.parseError(err)
        this.log.warn('[%s] %s %s.', linkedDevice.deviceName, this.lang.devNotAdd, eText)
    }
  }
  ...
  addAccessory (device) {
    ...
    try {
      ...
      accessory.context.linkedDevice = device.linkedDevice
      this.api.registerPlatformAccessories(plugin.name, plugin.alias, [accessory])
      this.configureAccessory(accessory)
        ...
        switch (params.cmd) {
          ...
          case 'brightness': {
            ...
            */
            data.apiParams = {
              cmd: 'brightness',
              data: this.consts.scaleAPIBrightness.includes(accessory.context.gvModel)
            ...
            data.awsParams = {
              cmd: 'brightness',
              data: {
                val: this.consts.noScaleAWSBrightness.includes(accessory.context.gvModel)
                  ? params.value
                  : Math.round(params.value * 2.54)
              }
            }
          ...
          case 'color': {
            ...
            */
            ...
            if (!this.consts.noAWSColour.includes(accessory.context.gvModel)) {
              data.awsParams = {
                cmd: 'color',
                data: {
                  red: params.value.r,
          ...
          case 'scene': {
            ...
            */
            if (params.command) {
              ...
              data.awsParams.data[params.value.charAt() === 'M' ? 'op' : 'opcode'] = 'mode'
            } else {
  ...
  receiveDeviceUpdate (accessory, params) {
    ...
    */
    if (this.funcs.hasProperty(params, 'brightness')) {
      data.brightness = this.consts.scaleAPIBrightness.includes(accessory.context.gvModel)
        ? Math.round(params.brightness / 2.54)
        : params.brightness
    } else if (params.cmd === 'brightness') {
      data.brightness = this.consts.noScaleAWSBrightness.includes(accessory.context.gvModel)
        ? params.data.val
    ...
    */
    if (params.cmd === 'ptReal') {
      data.command = JSON.stringify(params.data.command)
    } else if (['bulb', 'pt'].includes(params.cmd)) {
        data.scene =
