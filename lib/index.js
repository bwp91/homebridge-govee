...
class GoveePlatform {
  constructor (log, config, api) {
    ...
    try {
      ...
      const eText = err.message === this.lang.apiKeyMissing
        ? err.message
  ...
  async goveeSync () {
    try {
      ...
      deviceList.forEach(device => {
        devicesInGV.set(device.device, device)
      })

      // SYNC PART ONE
      // Initialise each Govee device into Homebridge
      devicesInGV.forEach(device => {
        try {
          ...
          if (!this.firstRunDone) {
            ...
            this.log('[%s] %s %s.', name, this.lang.devInit, device.device)
          }
      ...
      devicesInHB.forEach(async accessory => {
        try {
          ...
          if (!devicesInGV.has(accessory.context.gvDeviceId)) {
            this.removeAccessory(accessory)
            return
          }
