...
class GoveePlatform {
      ...
      const checkApiKey = async () => {
        try {
          ...
          if (this.funcs.isGoveeError(e)) {
            // Log the warning
            this.log.warn('%s.', this.messages.goveeInitErr)
  ...
  updateAccessoryStatus (accessory, newStatus) {
    ...
    if (newStatus) {
      this.log('[%s] %s.', accessory.displayName, this.messages.online)
    } else {
      this.log.warn('[%s] %s.', accessory.displayName, this.messages.offline)
    }
