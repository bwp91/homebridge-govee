...
class GoveePlatform {
      ...
      try {
        ...
        */
        if (['linux', 'freebsd', 'win32'].includes(require('os').platform())) {
          const socket = new (require('@homebridge/bluetooth-hci-socket'))()
  ...
  initialiseDevice (device) {
    ...
    try {
      ...
      // Temporarily override the debugging of the heater whilst in dev
      if (instance === 'heater') {
        accessory.context.enableLogging = true
        accessory.context.enableDebugLogging = true
      }
        ...
        switch (params.cmd) {
          ...
          case 'color': {
            ...
            */
            ...
            if (accessory.context.gvModel !== 'H6199') {
              data.awsParams = {
                cmd: 'color',
                data: {
                  red: params.value.r,
