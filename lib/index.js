...
class GoveePlatform {
  constructor (log, config, api) {
    ...
    this.funcs = new (require('./utils/functions'))(log, this.consts)
  ...
  applyUserConfig (config) {
    for (const [key, val] of Object.entries(config)) {
      switch (key) {
        ...
        case 'disablePlugin':
          this.config[key] = this.funcs.parseBool(key, val)
          break
        case 'ignoredDevices': {
          let newVal = val
          ...
          if (Array.isArray(newVal) && newVal.length > 0) {
            newVal.forEach(serialNumber => {
              this.config.ignoredDevices.push(this.funcs.parseSerialNumber(serialNumber))
            })
          }
          break
        }
        ...
        case 'platform':
          break
        case 'refreshTime':
          this.config.refreshTime = this.funcs.parseNumber(key, val)
          break
        default:
