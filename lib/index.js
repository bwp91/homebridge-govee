      ...
      deviceList.forEach(device => {
        try {
          ...
          if (this.consts.models.rgb.includes(device.model)) {
            ...
            instance =
              this.switchDevices[device.device] && this.switchDevices[device.device].showAsType
                ? this.switchDevices[device.device].showAsType
      ...
      devicesInHB.forEach(async accessory => {
        try {
          ...
          if (e.message.includes('400')) {
            if (accessory.enableDebugLogging) {
