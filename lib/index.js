...
class GoveePlatform {
    ...
    try {
      ...
      try {
        ...
        const eText = err.message.includes('401')
          ...
          : this.funcs.parseError(err, [this.lang.noDevList])
  ...
  initialiseDevice (device) {
    ...
    try {
      ...
      if (this.consts.models.rgb.includes(device.model)) {
        ...
        this.log.warn('[%s] %s:', device.deviceName, this.lang.devMaySupp)
      ...
      accessory.context.isRGBIC = deviceConf.isRGBIC
      accessory.context.lightEffects = deviceConf.lightEffects
  ...
  addAccessory (device) {
    ...
    try {
      ...
      accessory.context.linkedDevice = device.linkedDevice || false
      this.api.registerPlatformAccessories(plugin.name, plugin.alias, [accessory])
      this.configureAccessory(accessory)
  ...
  removeAccessory (accessory) {
    ...
    try {
      for (const uuid of Object.values(accessory.links | {})) {
        let linkedAccessory = devicesInHB.get(uuid)
        this.api.unregisterPlatformAccessories(plugin.name, plugin.alias, [linkedAccessory])
        devicesInHB.delete(uuid)
      }
  ...
  receiveDeviceUpdate (accessory, params) {
    ...
    */
    if (params.cmd === 'pt') {
      if (params.data.op === 'mode' || params.data.opcode === 'mode') {
        data.scene = params.data.value.join(',')
      }
    }
