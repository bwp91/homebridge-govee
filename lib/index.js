...
class GoveePlatform {
  ...
  initialiseDevice (device) {
    ...
    try {
      ...
      if (['heater', 'humidifier', 'purifier'].includes(instance)) {
        accessory.context.enableLogging = true
        accessory.context.enableDebugLogging = true
      }
      ...
      switch (params.cmd) {
        ...
        case 'color': {
          ...
          */
          ...
          } else if (!this.consts.noAWSColour.includes(accessory.context.gvModel)) {
            data.awsParams = {
              cmd: 'color',
              data: {
                red: params.value.r,
  ...
  receiveDeviceUpdate (accessory, params) {
    ...
    */
    ...
    } else if (params.cmd === 'colorwc' && params.data.color.colorTemInKelvin > 0) {
      data.kelvin = Math.max(Math.min(params.data.color.colorTemInKelvin, 7143), 2000)
    }
