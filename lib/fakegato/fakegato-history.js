...
module.exports = function (pHomebridge) {
  ...
  var hexToHPA = function (val) { // unused
    return parseInt(swap16(val), 10)
  }
  var hPAtoHex = function (val) { // unused
  ...
  class FakeGatoHistory extends Service {
    ...
    addEntry (entry) {
      switch (this.accessoryType) {
        ...
        case TYPE_MOTION:
        case TYPE_SWITCH:
          if (!this.disableTimer) { homebridge.globalFakeGatoTimer.addData({ entry: entry, service: this, immediateCallback: true }) } else { this._addEntry({ time: entry.time, status: entry.status }) }
          break
        case TYPE_AQUA:
          this._addEntry({ time: entry.time, status: entry.status, waterAmount: entry.waterAmount })
          break
        case TYPE_WEATHER:
          if (!this.disableTimer) { homebridge.globalFakeGatoTimer.addData({ entry: entry, service: this }) } else { this._addEntry({ time: entry.time, temp: entry.temp, humidity: entry.humidity, pressure: entry.pressure }) }
          break
        case TYPE_ROOM:
          if (!this.disableTimer) { homebridge.globalFakeGatoTimer.addData({ entry: entry, service: this }) } else { this._addEntry({ time: entry.time, temp: entry.temp, humidity: entry.humidity, ppm: entry.ppm }) }
          break
        case TYPE_ENERGY:
          if (!this.disableTimer) { homebridge.globalFakeGatoTimer.addData({ entry: entry, service: this }) } else { this._addEntry({ time: entry.time, power: entry.power }) }
          break
        case TYPE_CUSTOM:
          if (!this.disableTimer) {
