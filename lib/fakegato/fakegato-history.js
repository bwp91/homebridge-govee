  ...
  class FakeGatoHistory extends Service {
    constructor (accessoryType, accessory, optionalParams) {
      ...
      if (homebridge.globalFakeGatoStorage === undefined) {
        homebridge.globalFakeGatoStorage = new FakeGatoStorage({ log: this.log })
      }
          ...
          if (
            ...
            switch (this.accessoryType) {
              ...
              case 'custom': {
                ...
                for (const [k, v] of Object.entries(this.history[this.memoryAddress])) {
                  switch (k) {
                    case 'time':
                      break
                    default:
    ...
    setCurrentS2W1 (val, callback) {
      callback(null)
      this.log('[%s] FG history: data req [%s].', this.accessoryName, base64ToHex(val))
    ...
    setCurrentS2W2 (val, callback) {
      this.log('[%s] FG history: clock adj [%s].', this.accessoryName, base64ToHex(val))
      callback()
    }
  }
